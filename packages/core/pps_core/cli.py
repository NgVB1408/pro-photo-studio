...
from .inpaint import SUPPORTED_LAMA_MODELS, InpaintBackend, inpaint
from .mask import (
...
def _add_inpaint_options(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument(
        "--method", choices=["telea", "ns"], help="Phương pháp OpenCV inpaint (telea | ns)"
    )
    p.add_argument("--radius", type=int, help="Bán kính inpaint OpenCV (px)")
    p.add_argument(
        "--device",
    ...
    p.add_argument(
        "--lama-model",
        choices=list(SUPPORTED_LAMA_MODELS),
        default="lama",
    ...
    p.add_argument(
        "--crop-margin", type=int, default=196, help="Padding (px) quanh mask khi crop (LaMa)"
    )
    p.add_argument(
        "--crop-trigger-size",
        type=int,
...
def _add_mask_options(p: argparse.ArgumentParser) -> None:
    p.add_argument(
        "--auto", action="store_true", help="Tự phát hiện watermark (kết hợp text/bright/edge/logo)"
    )
    p.add_argument(
        "--auto-strategy",
        choices=["text", "bright", "edge", "logo", "auto"],
        default="auto",
        help="Chiến lược auto-detect: text=MSER, bright=logo trắng, "
        "edge=cạnh bất thường, logo=corner-logo, auto=tổng hợp",
    )
    p.add_argument(
        "--no-border-only", action="store_true", help="Khi --auto, KHÔNG giới hạn ở vùng rìa ảnh"
    )
    p.add_argument(
        "--box",
        action="append",
    ...
    p.add_argument(
        "--dilate", type=int, default=2, help="Số lần phồng mask (mở rộng vùng quanh logo)"
    )
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    rm.add_argument(
        "--keep-exif", action="store_true", help="Sao chép EXIF từ ảnh gốc (chỉ jpg/webp)"
    )
    ...
    au = sub.add_parser(
        "auto",
        help="Tự phát hiện + xoá watermark (không cần box/mask)",
    )
    ...
    au.add_argument(
        "--no-border-only", action="store_true", help="KHÔNG giới hạn vùng rìa khi tự phát hiện"
    )
    au.add_argument("--dilate", type=int, default=4, help="Số lần phồng mask")
    ...
    bt.add_argument("--recursive", action="store_true", help="Đệ quy xuống các thư mục con")
    bt.add_argument("--workers", type=int, help="Số process song song")
    bt.add_argument(
        "--suffix", default="_clean", help="Hậu tố thêm vào tên file đầu ra (vd: _clean)"
    )
    bt.add_argument(
        "--out-format", help="Định dạng đầu ra (.jpg/.png/.webp); mặc định giữ định dạng gốc"
    )
    bt.add_argument(
        "--no-skip-existing", action="store_true", help="Ghi đè file đã tồn tại (mặc định bỏ qua)"
    )
    ...
    vd.add_argument(
        "--mask-mode",
        choices=["static", "auto", "redetect"],
        default="static",
        help="static: 1 mask cho mọi frame. auto: detect frame đầu, "
        "tái dùng. redetect: detect mỗi frame (chậm).",
    )
    vd.add_argument("--max-frames", type=int, help="Giới hạn N frame đầu (debug)")
    ...
    pt.add_argument(
        "--init-from-auto", action="store_true", help="Khởi tạo mask từ kết quả tự phát hiện"
    )
    ...
    en = sub.add_parser(
        "enhance",
        help=("Cải thiện ảnh (cân bằng trắng / exposure / vibrance / sharpen) — không cần ML"),
    )
    ...
    en.add_argument("--quality", type=int, default=92, help="Chất lượng JPEG/WebP 1..100")
    en.set_defaults(func=cmd_enhance)

    # batch-enhance
    eb = sub.add_parser(
        "batch-enhance",
        help="Cải thiện cả thư mục ảnh",
    )
    eb.add_argument("source", help="Thư mục hoặc glob pattern")
    eb.add_argument("-o", "--out-dir", required=True, help="Thư mục đầu ra")
    eb.add_argument("--recursive", action="store_true", help="Đệ quy xuống các thư mục con")
    eb.add_argument(
        "--preset",
        choices=["studio", "real_estate", "portrait", "product", "outdoor"],
        default="studio",
    ...
    eb.add_argument("--workers", type=int, help="(Hiện tại xử lý tuần tự — flag dự phòng)")
    eb.add_argument("--suffix", default="_enhanced", help="Hậu tố thêm vào tên file đầu ra")
    ...
    sky = sub.add_parser(
        "replace-sky",
        help="Thay trời xám bằng trời xanh / hoàng hôn / nhiều mây",
    )
    ...
    sky.add_argument("--sky-image", help="Ảnh trời thay thế tuỳ chọn (ưu tiên hơn preset)")
    sky.add_argument("--blend", type=float, default=1.0, help="Mức trộn 0..1 (1 = thay 100%%)")
    sky.add_argument("--feather", type=int, default=21, help="Kernel làm mềm rìa mask (px)")
    ...
    wp = sub.add_parser(
        "window-pull",
        help="Kéo sáng cửa sổ bị cháy (ảnh nội thất)",
    )
    wp.add_argument("input", help="Đường dẫn ảnh nội thất")
    wp.add_argument("-o", "--output", required=True, help="Đường dẫn ảnh đầu ra")
    wp.add_argument("--strength", type=float, default=0.7, help="Cường độ 0..1 (1 = mạnh nhất)")
    wp.add_argument("--quality", type=int, default=95)
    wp.set_defaults(func=cmd_window_pull)

    lw = sub.add_parser(
        "lawn-enhance",
        help="Tăng độ tươi cỏ (selective HSV)",
    )
    lw.add_argument("input", help="Đường dẫn ảnh đầu vào")
    lw.add_argument("-o", "--output", required=True, help="Đường dẫn ảnh đầu ra")
    lw.add_argument("--sat-boost", type=float, default=0.5, help="Tăng độ bão hoà 0..1")
    lw.add_argument("--hue-shift", type=int, default=-3, help="Dịch hue (âm = về xanh tươi)")
    lw.add_argument("--value-lift", type=float, default=0.08, help="Nâng vùng tối trong cỏ 0..1")
    lw.add_argument("--quality", type=int, default=95)
    lw.set_defaults(func=cmd_enhance_lawn)

    vc = sub.add_parser(
        "correct-vertical",
        help="Kéo thẳng đường dọc (sửa nghiêng phối cảnh)",
    )
    vc.add_argument("input", help="Đường dẫn ảnh")
    vc.add_argument("-o", "--output", required=True, help="Đường dẫn ảnh đầu ra")
    vc.add_argument(
        "--max-angle", type=float, default=8.0, help="Chỉ sửa khi |độ nghiêng| ≤ giá trị này (độ)"
    )
    vc.add_argument("--no-crop", action="store_true", help="KHÔNG crop biên đen sau khi xoay")
    vc.add_argument("--quality", type=int, default=95)
    vc.set_defaults(func=cmd_correct_vertical)

    cs = sub.add_parser(
        "classify",
        help="Tự tag cảnh: nội thất / ngoại thất / trên cao (không cần ML)",
    )
    cs.add_argument("input", help="Đường dẫn ảnh")
    cs.add_argument("--json", action="store_true", help="Xuất kết quả ở dạng JSON")
    ...
    re.add_argument("--no-window", action="store_true", help="Tắt kéo sáng cửa sổ")
    re.add_argument("--no-lawn", action="store_true", help="Tắt tăng tươi cỏ")
    re.add_argument("--no-vertical", action="store_true", help="Tắt kéo thẳng đường dọc")
    ...
    cp.add_argument(
        "--no-align", action="store_true", help="Bỏ tự căn chỉnh ORB (dùng khi 2 ảnh đã khít)"
    )
    cp.add_argument("--threshold", type=int, default=25, help="Ngưỡng pixel diff (mặc định 25)")
    cp.add_argument("--feather", type=int, default=3, help="Làm mềm rìa mask khi blend (px)")
    ...
    ev.add_argument("--json", action="store_true", help="Xuất kết quả ở dạng JSON")
    ...
    sv.add_argument(
        "--share", action="store_true", help="Tạo public URL qua gradio (lưu ý về privacy)"
    )
    ...
    us.add_argument("--per-page", type=int, default=10, help="Số kết quả mỗi trang (1..30)")
    us.add_argument("--page", type=int, default=1, help="Số trang")
    us.add_argument(
        "--orientation", choices=["landscape", "portrait", "squarish"], help="Lọc theo bố cục"
    )
    ...
    ud.add_argument(
        ...
        default="raw",
        help="Kích thước (raw = chất lượng cao nhất)",
    )
    ...
    dl.add_argument("--recursive", action="store_true", help="Đệ quy xuống các thư mục con")
    ...
    dd.add_argument("--recursive", action="store_true", help="Đệ quy khi tải folder")
    dd.add_argument("--out", default="downloads", help="Thư mục lưu")
    dd.set_defaults(func=cmd_dropbox_download)

    pl = sub.add_parser("pipeline", help="Tải ảnh từ Unsplash rồi (tuỳ chọn) xoá watermark")
    ...
    pl.add_argument("--size", default="raw", choices=["raw", "full", "regular", "small", "thumb"])
    pl.add_argument("--no-clean", action="store_true", help="Chỉ tải, không xoá watermark")
...
def _build_mask_from_args(args: argparse.Namespace, image):
    ...
    if getattr(args, "color_lower", None) is not None:
        masks.append(
            build_mask_from_color(
                image,
                lower=args.color_lower,
                upper=args.color_upper,
            )
        )
    if not masks:
        raise SystemExit("Cần ít nhất một nguồn mask: --auto, --box, --mask, hoặc --color-lower")
...
def cmd_batch(args: argparse.Namespace, settings: Settings) -> int:
    ...
    if result.failed:
        for _path, _err in result.failed[:10]:
            pass
...
def cmd_replace_sky(args: argparse.Namespace, settings: Settings) -> int:
    ...
    from .realestate import load_sky_from_path, replace_sky
...
def cmd_unsplash_download(args: argparse.Namespace, settings: Settings) -> int:
    ...
    if args.id:
        ...
        return 0
    if args.query:
        photos = client.search(args.query, per_page=args.count, orientation=args.orientation)
        if not photos:
